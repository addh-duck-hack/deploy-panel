...
- **`src/configFiles.js`** — `findConfigFiles(projectPath)` walks a single project's directory (no caching, re-scanned per request, same philosophy as `scanProjects()`) looking for editable config files — currently `.env`/`.env.*` and Angular `environment*.ts`, matched by basename regex. Skips `node_modules`, `vendor`, `dist`, `build`, `coverage`, `tmp`, `wp-content`, and any dot-directory; does not follow symlinked directories (cycle safety, not access control — this app already has root-equivalent host access by design). Bounded by `MAX_DEPTH` and `MAX_ENTRIES` so a pathological tree can't hang a request. The `GET/POST /api/projects/:name/files` routes treat this function's freshly-recomputed output as the *only* whitelist of paths a client is allowed to read/write — there's no separate path-traversal check because a relative path can only ever appear in the list if the scanner itself just found it on disk.
