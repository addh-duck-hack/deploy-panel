...
- **`src/jobs.js`** — Runs and tracks deploys in-memory (no DB, no persistence across restarts):
  - `DEPLOY_STEPS` is the fixed, ordered command list: `git pull --ff-only`, `docker compose down`, `docker compose up -d --build`. **Order matters**: pull goes first on purpose so a failed pull never touches the running stack (zero downtime on pull failure).
